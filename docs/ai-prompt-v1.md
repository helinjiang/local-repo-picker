...
• 最大深度：maxDepth（默认 7）
• 忽略目录：
• pruneDirs: string[]（如 node*modules / dist / build 等）
• basename 以 . 或 * 开头的目录
...
🧪 验收标准（请自检）1. npm i -g . 后可运行 repo 2. repo --config 能创建并提示 config 路径 3. UI 能列出多个 scanRoots 下的 repo 4. 上下移动时右侧预览实时变化（含 commit 时间到秒）5. Enter 后 CLI 至少打印 repo path 6. 在 Node 项目中可：
