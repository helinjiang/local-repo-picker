...
function normalizeConfig(raw: unknown): AppConfig {
  ...
  const maxDepth = typeof value.maxDepth === 'number' ? value.maxDepth : defaultConfig.maxDepth;
  const pruneDirs = Array.isArray(value.pruneDirs)
    ? value.pruneDirs.filter((item) => typeof item === 'string')
    : defaultConfig.pruneDirs;
  const cacheTtlMs =
    typeof value.cacheTtlMs === 'number' ? value.cacheTtlMs : defaultConfig.cacheTtlMs;
  const followSymlinks =
    typeof value.followSymlinks === 'boolean' ? value.followSymlinks : defaultConfig.followSymlinks;
  ...
  const fzfTagFilters =
    typeof value.fzfTagFilters === 'object' && value.fzfTagFilters !== null
      ? (Object.fromEntries(
          Object.entries(value.fzfTagFilters as Record<string, unknown>).filter(
            ([key, item]) => typeof key === 'string' && typeof item === 'string',
          ),
        ) as Record<string, string>)
