...
export async function buildCache(
  ...
  const repoTasks = found.map((repo) =>
    repoLimit(async () => {
      ...
      const autoTags = uniqueTags([...baseAutoTags, ...extraTags]).filter(
        (tag) => !removed.has(tag),
      );
...
export async function refreshCache(options: ScanOptions): Promise<CacheData> {
...
function normalizeOptions(options: ScanOptions): ScanOptions & {
  maxDepth: number;
  pruneDirs: string[];
  cacheTtlMs: number;
...
function normalizeCacheRepos(
  input: Array<RepositoryRecord | Record<string, unknown>>,
): RepositoryRecord[] {
  return input
    .map((item) => {
...
async function persistCache(cacheFile: string, cache: CacheData): Promise<void> {
