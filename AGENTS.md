...
- 当使用 React 实现新功能时，尽量考虑用 hooks 封装逻辑，以及用组件封装视图（优先考虑基于 antd 组件），提升代码可维护性与可测试性
- src 内部导入路径不写 .js 后缀，构建阶段自动补全
- ESM 为主，避免引入 CommonJS 风格写法
- 避免 any
- 日志统一使用 core/logger，DEBUG=1 才输出详细信息
- if 必须使用大括号包裹，即使只有一行语句
- if 前后保持空行，try...catch 前后保持空行，return 之前保持空行
- 每次生成或修改代码后必须执行格式化并确保满足上述空行与大括号规则
