...
describe("core origin/preview/command", () => {
  ...
  it("readOriginValue 回退到 runGit", async () => {
    ...
    vi.mocked(gitMocks.runGit).mockResolvedValue({ ok: true, stdout: "https://github.com/a/b.git\n" })
    const value = await readOriginValue("/tmp/repo")
  ...
  it("buildRepoPreview 在完整 git 信息时输出预览", async () => {
    ...
    vi.mocked(pluginMocks.resolvePreviewExtensions).mockResolvedValue([{ title: "EXT", lines: ["ok"] }])
  ...
  it("runCommand 执行命令并处理异常参数", async () => {
    ...
    await expect(runCommand("node", ["-e", "console.log('ok')"], { cwd: "\0" })).rejects.toThrow("invalid path")
  })
})
